package com.line.demo.opengl.panorama;
...
public class Ball implements GLSurfaceView.Renderer {
    ...
    public Ball(Context context, int imageId) {
        mContext = context;
        mImageId = imageId;
        init();
    }
    ...
    private void init() {
        ...
        List<Float> vetexList = new ArrayList<>();
        ...
        vertexBuff =
                ByteBuffer.allocateDirect(vetex.length * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();
    ...
    @Override public void onSurfaceChanged(GL10 gl, int width, int height) {
        ...
        textrueID = OGLUtils.initTexture(mContext, mImageId);
